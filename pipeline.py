...
def read_system_files(positions_file_path, parameters_file_path, system_options, gromacs_include_dir=None, charmm_parameter_files=None): # Added by JBodosa on 1stOct2024 
    ...
    # Read CHARMM format psf and pdb files
        ...
        # ##system = openmm.XmlSerializer.deserialize(serialized_system)
        #a =openmm.vec3.Vec3(16, 0, 0)*unit.nanometer
        #b =openmm.vec3.Vec3(0, 16, 0)*unit.nanometer
        #c =openmm.vec3.Vec3(0, 0, 16)*unit.nanometer
        #sys = openmm.openmm.System() 
        #sys.setDefaultPeriodicBoxVectors(a,b,c)
        # ## Added until
        #
        # gro files must contain box vectors, so we must determine whether system
        # is non-periodic or not from provided nonbonded options
        #box_vectors = positions_file.topology.getPeriodicBoxVectors()
        parameters_file.setBox(6, 6, 6) # 3nm
        box_vectors = parameters_file.topology.getPeriodicBoxVectors()
        #        box_vectors = sys.getPeriodicBoxVectors()
        # WARNING: This uses the private API for GromacsGroFile, and may break.
        if ('nonbonded_method' in system_options and
                system_options['nonbonded_method'] in _NONPERIODIC_NONBONDED_METHODS):
            logger.info('nonbonded_method = {}, so removing periodic box vectors '
                        'from pdb file'.format(system_options['nonbonded_method']))
            box_vectors = None
